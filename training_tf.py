...
def preprocess():
    ...
    labs = np_utils.to_categorical(labs, 34)

    # 隨機劃分測試集與訓練集
    train_x, test_x, train_y, test_y = train_test_split(imgs, labs, test_size=0.2,
                                                        random_state=0)  # 請改test_size (Change Me)
...
def train():
    ...
    num_classes = 34  # 分成62種 0~9 a~z A~Z
    ...
    train_history = model.fit(train_x, train_y, epochs=epochs, batch_size=batch_size, verbose=2,
